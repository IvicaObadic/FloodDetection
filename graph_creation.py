...
def parse_args():
    ...
    parser.add_argument(
        '--num_SLIC_segments', type=int, default=100,
...
def create_and_save_SIFT_graphs(image_folder,  flood_label, save_dir):
    ...
    for i, img_id in enumerate(images):
        ...
        graph_data_positions_similarity = Data(torch.from_numpy(des), pos=torch.from_numpy(kp_positions), y=flood_label)
        ...
        graph_data_embeddings_similarity = Data(torch.from_numpy(des), edge_index=edge_index, y=flood_label)
...
def create_SLIC_graphs(image_folder,  flood_label, save_dir, n_segments=1000):
    ...
    save_dir = save_dir + "{}_segments".format(n_segments)
...
def create_graphs(args):
    ...
    for i, class_name in enumerate(class_names):
        ...
        if encoding_method == "SIFT":
            create_and_save_SIFT_graphs(images_folder, i, graphs_dir)
